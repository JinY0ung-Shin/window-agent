...
export function FolderWhitelist() {
  ...
  useEffect(() => {
    if (selectedAgentId) {
      fetchFolderWhitelist(selectedAgentId);
    }
  }, [selectedAgentId, fetchFolderWhitelist]);

  const handleAdd = async () => {
    if (!selectedAgentId || !newPath.trim()) return;
    await addFolder(selectedAgentId, newPath.trim());
    setNewPath("");
  };
  ...
  const handleRemove = async (folderId: string) => {
    ...
    await removeFolder(selectedAgentId, folderId);
  };
        ...
        <>
          <div className="flex gap-2 mb-4">
            <input
              ...
              placeholder="폴더 경로 입력 (예: /home/user/documents)"
          ...
          <div className="bg-surface-700/40 rounded-xl">
            ...
            ) : folderWhitelist.length > 0 ? (
              <ul className="divide-y divide-white/[0.04]">
                {folderWhitelist.map((entry) => (
                  <li
                    key={entry.id}
