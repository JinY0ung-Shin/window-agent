...
pub fn seed_all_agents(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    // Migrate existing agents with empty api_key from claude back to openai
    conn.execute(
        "UPDATE agents SET ai_backend = 'openai' WHERE ai_backend = 'claude' AND (api_key IS NULL OR api_key = '')",
        [],
    )?;
