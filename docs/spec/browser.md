...
| method | 설명 | 주요 동작 |
|--------|------|-----------|
| `create_session` | 세션 생성 | `browser.newContext({ viewport: null })` + `newPage()`. 뷰포트는 브라우저 창 크기에 동적 연동 |
...
| `scroll` | 페이지 스크롤 | `page.mouse.wheel(x, y)` + 300ms 대기 |
| `key` | 키보드 입력 | `page.keyboard.press(key)` + 300ms 대기 |
| `select_option` | 드롭다운 선택 | `locator.selectOption(value)` |
| `hover` | 요소 호버 | `locator.hover()` |
| `handle_dialog` | 다이얼로그 처리 | `dialog.accept()`/`dialog.dismiss()`. 5초 타임아웃 |
| `tabs` | 탭 관리 | list/create/close/select. `context.newPage()`/`page.close()` |
| `evaluate` | JS 실행 | `page.evaluate(expression)` |
...
| `browser_scroll` | auto | 360s | 페이지 스크롤 (x/y 픽셀) |
| `browser_key` | auto | 360s | 키보드 키 입력 (Enter, Escape 등) |
| `browser_select_option` | confirm | 360s | `<select>` 드롭다운에서 옵션 선택 |
| `browser_hover` | auto | 360s | 요소에 호버 (툴팁/드롭다운 트리거) |
| `browser_handle_dialog` | confirm | 360s | alert/confirm/prompt 다이얼로그 처리 |
| `browser_tabs` | confirm | 360s | 탭 관리 (list/create/close/select) |
| `browser_evaluate` | confirm | 360s | JavaScript 표현식 실행 |
| `browser_close` | confirm | 360s | 세션 종료 |

### 11.1.1 Playwright MCP 대비 커버리지

> 참고: `@playwright/mcp` (Microsoft 공식, npm) 대비 비교

| 카테고리 | 우리 | Playwright MCP | 비고 |
|---|---|---|---|
| 기본 (navigate, click, type, snapshot, wait, back) | O | O | |
| scroll | O | O | `page.mouse.wheel` |
| key press | O | O | `page.keyboard.press` |
| hover | O | O | `locator.hover` |
| select option | O | O | `locator.selectOption` |
| dialog 처리 | O | O | `page.once('dialog')` |
| 탭 관리 | O | O | `context.pages()` |
| JS 실행 | O | O | `page.evaluate` |
| drag & drop | X | O | 극히 드문 케이스 |
| file upload | X | O | 필요시 추가 |
| resize | 불필요 | O | `viewport: null`로 동적 연동 |
| 좌표 기반 클릭 (vision) | X | O (opt-in) | accessibility tree로 대부분 커버 |
| 네트워크 mock/cookie/storage | X | O (15개+) | 테스트 도구 영역, 에이전트에 불필요 |

**설계 차이**: Playwright MCP와 동일하게 accessibility tree + ref 번호 기반 상호작용. 픽셀/비전 모델 불필요.
별도의 보안 계층(도메인 승인, credential 주입/리다크션, 비밀번호 필드 보호)이 추가되어 있음.
...
| `src-tauri/src/browser/commands.rs` | navigate/snapshot/click/type/wait/back/scroll/key/select_option/hover/handle_dialog/tabs/evaluate 실행 |
